...
Shop::Shop() : m_name("冒险者商店") {
    ...
    addItem(Item("铁剑", "攻击+5", ItemType::EQUIPMENT_WEAPON, 80, 0, 1, 5, 0, 0));
    addItem(Item("皮甲", "防御+3", ItemType::EQUIPMENT_ARMOR, 60, 0, 1, 0, 3, 0));
    addItem(Item("速度之靴", "速度+10", ItemType::EQUIPMENT_ACCESSORY, 100, 0, 1, 0, 0, 10));
}
...
bool Shop::buyItem(Character& character, const string& itemName, int quantity) {
    ...
    if (character.getGold() < totalPrice) {
        cout << "❌ 金币不足！需要 " << totalPrice << " 金币，当前有 " << character.getGold() << " 金币" << endl;
        return false;
    }

    if (shopItem->getQuantity() < quantity) {
        cout << "❌ 库存不足！当前库存: " << shopItem->getQuantity() << endl;
        return false;
    }
    ...
    if (shopItem->getQuantity() <= 0) removeItem(itemName);

    cout << "✅ 成功购买 " << quantity << " 个 " << itemName << "，花费 " << totalPrice << " 金币" << endl;
    return true;
}
